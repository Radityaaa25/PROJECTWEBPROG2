    ...
    <div class="col-md-5">
        <div class="glass-card shadow-lg border-0" style="backdrop-filter: blur(20px); border-radius: 25px; overflow: hidden;">
            <div class="card-body p-5">
                ...
                @if(session('error'))
                    <div class="alert alert-danger rounded-pill text-center border-0 shadow-sm">{{ session('error') }}</div>
                @endif
                ...
                <div class="mt-4 text-center">
                    <p class="text-muted">Belum punya akun? <a href="{{ route('frontend.register') }}" class="text-primary fw-bold text-decoration-none">Daftar di sini</a></p>
                    <p class="text-muted small mt-3">Admin toko? <a href="{{ route('backend.login') }}" class="text-secondary text-decoration-none">Login di sini</a></p>
                </div>
            </div>
        </div>
    </div>
